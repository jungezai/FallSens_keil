...
//	uint8_t addr= MPU6050_ADDRESS << 1;
  ...
  *pACC_X = (buf[0] << 8) | buf[1];
  ...
  if(*pACC_Y & 0x8000) *pACC_Y-=65536;
	//*pACC_Y = (int16_t)2*9.8*(*pACC_Y/32768);
	
  *pACC_Z = (buf[4] << 8) | buf[5];
	if(*pACC_Z & 0x8000) *pACC_Z-=65536;
	//*pACC_Z = (int16_t)2*9.8*(*pACC_Z/32768);
}

void MPU6050_ReadTmp( float *tmp)
{
		uint8_t buf[2];
		short raw;
//		uint8_t addr= MPU6050_ADDRESS << 1;
		mpu6050_register_read(MPU6050_TMP_OUT, buf, 2);
