...
{
    p_hts->conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
}
...
uint32_t encoded_temp[3];
    ...
    if (p_hts->conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        hvx_params.p_data = encoded_hts_meas;
			  ...
			  encoded_temp[0] = (((-2) << 24) & 0xFF000000) |
                       (((int32_t)(temp1[current_index][0]*100) <<  0) & 0x00FFFFFF);
			uint32_encode(encoded_temp[0],&p_buffer[1]);
				encoded_temp[1] = (((-2) << 24) & 0xFF000000) |
                       (((int32_t)(temp1[current_index][1]*100) <<  0) & 0x00FFFFFF);
			uint32_encode(encoded_temp[1],&p_buffer[5]);
				encoded_temp[2] = (((-2) << 24) & 0xFF000000) |
                       (((int32_t)(temp1[current_index][2]*100) <<  0) & 0x00FFFFFF);
			uint32_encode(encoded_temp[2],&p_buffer[9]);
			hvx_len = 13;
			if(SAMPLE_SIZE != 1 ) {
						if(current_index == 0)
			{
				memset(p_buffer,0,13);
			} 
